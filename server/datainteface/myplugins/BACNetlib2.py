...
# -*- coding: utf-8 -*-
#
#2016.3.21 修改自BACnet-discover-enumerate.nse
#
#
...
def vendornum_query(socket, TargetAddr):
    ...
    if data.startswith('\x81',0): #响应以0x81起始，说明是BACNet
        print 'successful'
    #print 'value:', repr(data)
    #print 'len(data)', len(data)
   #print 'type(data)',type(data)
    #value = data[6]     #读数据包第7个字符
    value = 0
    ...
    if value == '21':
        print int(b2a_hex(data[18:19]),16)
        #print string.atoi(data[18],16)
        return vendor_lookup(ord(data[18]))
    elif value == '22':
        print int(b2a_hex(data[18:19]),16)
        #print data[18].decode('utf8')
        print ord(data[18])     #取字符的整数值
        
        (vendornum,) = struct.unpack('>H', data[18:20])
        
        #print string.atoi(data[18].decode('utf8'),10)
    ...
    '''
    print (value)
    print 'data[17]', '50'.decode('hex')
    if value != '50'.decode('hex'):
        print 'xxx'
    #for i in (data):
        #print (i)
        #print ord(i)
    value = data[17]
    #print ord(value)  #取字符的整数值
    print b2a_hex(value)
    if b2a_hex(value)
    ...
    print int('0x22',16)
    '''
    #if int(value) == '22'.decode('hex'):
    #if int(value) == int('0x22',16):    
        #print '22'
def field_size(data):
    value = ord(data[17])
    if value % 16 < 5:
...
def GetInfo(TargetAddr, s, data):
    ...
    address = ("", 0)
    TargetAddr = (IP, port)
    #TargetAddr = ("108.230.13.161", port)
    #TargetAddr = ("162.127.5.4", port)
    ...
    try:
        ...
        print to_return["VendorID"]
    except Exception,e:
        print 'error:', e
        to_return['VendorID'] = str(e)
    
    to_return["VendorName"] = standard_query(s, "vendor", TargetAddr)
    print to_return["VendorName"]
  
    
    #print ord(data[19:22])
    print int(b2a_hex(data[19:22]),16)
    
    #print data[18].decode('utf8')
    
    #(id,) = struct.unpack('!3s', '\x3f\xff\xff')  
    #print b2a_hex(id)
    ...
    #if vendornum_query(s, TargetAddr) != 0:
        #to_return["VendorID"] = vendornum_query(s, TargetAddr)
        #print to_return["VendorID"]
    #else:
        #print 'get VendorID error'
        #to_return['VendorID'] = 'get VendorID error'
        #return to_return    
    print to_return
    return to_return
